...
#
# Note that we will refer to the original model as ``BiGRU-with-attention'', as it combines a Bidirectional 
# GRU encoder with the use of attention in the decoder.
# > In these exercises you will test different variants of the BiGRU-with-attention model. 
# >In these exercises themn specifically, you will create the following models:
# >>    1. BiLSTM-with-attention: A nearly identical model in which the the (Bi)GRU is replaced with a BiLSTM
# >>    2. BiGRU-without-attention: A similar BiGRU model, but without attention. Instead of using a weighted mean of the hidden states of the 
# >>       encoder, it just always uses the last state of the encoder.
# >>    3. BiLSTM-without-attetion: similar to 2, but using again a BiLSTM in place of a BiGRU.
#
...
#
# Note that most of the work will be to make the adaptations for implementing models 1 and 2, once you have these, 
# model 3 should be straightforward as it is just a simple combination of the two adaptations.
#
# Tips: 
